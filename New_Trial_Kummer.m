function [ y ] = New_Trial_Kummer(mu)
...
% for alpha i
     ...
     alpha(i) = exp(-1*c./2.*((sqrt(2)*std_dev.*(time(i))) + mu(j))); % mu == mean
     ...
     for i= 1:length(time)
              ...
              out1 = 0;
              for L = 0:u-1
              out1 = (((lambda)^L)*(exp(-1*(lambda./2))))./((gamma(L+1))*(2^L)) + out1;
              end
   
              out2  = 0;
              for k = 0:10
              out2  = ((lambda.^u)*((-1).^k)*gamma(1+c*k/2)*((A*alpha(i)).^k)*kummer((1+c*k)/2, u+1, lambda/2))/((2.^(u/2))*(2.^(c*k/2))*gamma(k+1)*(exp(lambda/2))*gamma(u+1)) + out2 ;
              end
out2
      y = W(i)*(out1 + out2) + y;
     end
