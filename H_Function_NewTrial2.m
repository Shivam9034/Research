function [ y ] = H_Function_NewTrial2(an,An,ap, Ap,bm, Bm, bq, Bq, Z )
...
syms s;
%num = (for_m(bm, Bm)).*(for_n(an, An)).*(Z.^(-s));
%num
%den = (for_q(bq, Bq)).*(for_p(ap, Ap));
%den
%oout = num./den;
%oout
fun  = @(s) ((for_m(bm, Bm)).*(for_n(an, An)).*(Z.^(-s))./((for_q(bq, Bq)).*(for_p(ap, Ap)))); 
fun
y = integral(fun, 2, 3 );
...
if n == 0
    ...
    %syms s;
    for k = 1:n
    Out_n = (gamma(1 - an(k) - An(k).*x)).*(Out_n); 
    end
end
Out_n
end
...
%syms s;
for k = 1 : q
    Out_q = (gamma(1 - bq(k) - Bq(k).*x)).*(Out_q); 
end
end
Out_q
end
